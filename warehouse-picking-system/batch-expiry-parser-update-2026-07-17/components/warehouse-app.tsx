...
import type { CurrentProfile, DatabaseOrderStatus, LiveOrder, ParsedOrder, ParseSummary } from "@/lib/types";
...
export function WarehouseApp() {
  ...
  const parsePdf = async () => {
    ...
    try {
      ...
      const result = await response.json() as { order?: ParsedOrder; summary?: ParseSummary; error?: string };
  ...
  return (
    <div className="app-shell">
      ...
      {uploadOpen && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Upload Unleashed order"><div className="modal"><div className="modal-head"><div><h2>Import Unleashed order</h2><p>Extract the order for review, then import it into the live picking queue.</p></div><button className="close-button" aria-label="Close" onClick={closeUpload}>×</button></div><div className="dropzone"><input type="file" accept="application/pdf" onChange={(event) => { setFile(event.target.files?.[0] ?? null); setParsed(null); setParseSummary(null); setError(""); setImportSuccess(""); }} /></div>{error && <p style={{ color: "#b8443c" }}>{error}</p>}{parsed && !importSuccess && <div className="parse-review"><strong>{parsed.orderNumber}</strong> · {parsed.customer}<div className="parse-review-stats"><span><b>{parseSummary?.pages ?? "—"}</b> pages</span><span><b>{parseSummary?.lineItems ?? parsed.items.length}</b> lines</span><span><b>{parseSummary?.units ?? parsed.items.reduce((total, item) => total + item.quantity, 0)}</b> units</span><span><b>{parseSummary?.batchAllocations ?? parsed.items.reduce((total, item) => total + (item.batches?.length ?? 0), 0)}</b> batches</span></div>{Boolean(parseSummary?.itemsWithoutBatch) && <p className="parse-warning">{parseSummary!.itemsWithoutBatch} product line{parseSummary!.itemsWithoutBatch === 1 ? " has" : "s have"} no batch or expiry supplied in the PDF.</p>}<p className="parse-ok">Validated: every detected order-quantity row is included.</p></div>}{importSuccess && <div style={{ border: "1px solid #a9d9c8", color: "#116149", background: "#edf9f4", borderRadius: 10, padding: 13, marginBottom: 16, fontSize: 11, fontWeight: 700 }}>{importSuccess}</div>}<div className="modal-actions"><button className="ghost-button" onClick={closeUpload}>Cancel</button>{parsed && !importSuccess && <button className="ghost-button" disabled={parsing || importing} onClick={parsePdf}>Re-read PDF</button>}<button className="primary-button" disabled={(!file || parsing || importing) && !importSuccess} onClick={importSuccess ? viewImportedOrder : parsed ? importOrder : parsePdf}>{parsing ? "Reading PDF…" : importing ? "Saving order…" : importSuccess ? "View order queue" : parsed ? "Import to warehouse" : "Extract order"}</button></div></div></div>}
    </div>
  );
}
