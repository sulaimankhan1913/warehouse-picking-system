...
export async function POST(request: Request) {
  ...
  const { error: itemError } = await supabase.from("order_items").insert(order.items.map((item, index) => ({
  ...
  const { error: auditError } = await supabase.from("audit_logs").insert({
    ...
    details: { order_number: order.orderNumber, source_pdf_path: storagePath, line_items: order.items.length },
  });
