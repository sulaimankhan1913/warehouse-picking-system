...
export async function GET() {
  ...
  if (profileResult.error) {
    return NextResponse.json({ error: "Your warehouse profile could not be loaded." }, { status: 403 });
  }
  if (orderResult.error) {
    console.error("Order queue load failed", orderResult.error);
    return NextResponse.json({ error: "The order queue could not be loaded." }, { status: 500 });
  }
  ...
  return NextResponse.json({
    ...
    orders: ((orderResult.data ?? []) as unknown as OrderRow[]).map(toLiveOrder),
  });
}
